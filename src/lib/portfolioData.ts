...
// Resume URL helpers - stores the Firebase Storage download URL in Realtime DB
export const getResumeUrl = async (): Promise<{ url: string; uploadedAt: number } | null> => {
  try {
    const resumeRef = ref(db, `${DATA_PATH}/resume`);
...
export const saveResumeUrl = async (url: string, uploadedAt: number) => {
  try {
    const dataRef = ref(db, DATA_PATH);
    await update(dataRef, {
